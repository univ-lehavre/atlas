/**
 * `from-crf` command — fetches researchers from REDCap and resolves their OpenAlex works.
 */
...
export const fromCrf = async (
  opts: FromCrfOptions,
): Promise<readonly ResearcherRow[]> => {
  const crfConfig: CrfConfig = {
    url: opts.crfUrl,
    token: opts.crfToken,
  };
  ...
  const fetchResult: Either.Either<readonly ResearcherRow[], unknown> =
    await Effect.runPromise(Effect.either(fetchResearchers(crfConfig)));
