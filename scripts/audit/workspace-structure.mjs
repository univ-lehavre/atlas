...
const ROOTS = ['apps', 'cli', 'config', 'packages', 'sandbox', 'services', 'ui'];
...
const CLI_IO_MIGRATION_PENDING = new Set(['@univ-lehavre/atlas-validate-openalex']);

// Deps that belong in cli/, never in packages/ dependencies
const CLI_IO_DEPS = new Set(['@clack/prompts', 'yargs', 'commander', 'meow', 'inquirer', 'prompts']);
...
const TERMINAL_NODE_MODULES = ['node:readline', 'node:tty', "'readline'", "'tty'"];
...
function* iterSourceFiles(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (entry.name.endsWith('.ts') || entry.name.endsWith('.js')) yield full;
  }
}

function findForbiddenImports(srcDir, forbidden) {
  const hits = [];
  for (const file of iterSourceFiles(srcDir)) {
    const content = readFileSync(file, 'utf8');
    for (const match of content.matchAll(IMPORT_RE)) {
      if (forbidden.includes(match[1])) {
        hits.push({ file: file.replace(process.cwd() + '/', ''), module: match[1] });
      }
    }
  }
  return hits;
...
for (const root of ROOTS) {
  ...
  for (const entry of readdirSync(root)) {
    ...
    if (packageJson.name) workspaces.set(packageJson.name, { root, dir, packageJson });
  }
}
...
function detectCycles() {
  ...
  for (const [name, { packageJson }] of workspaces) {
    const allDeps = deps(packageJson, 'dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies');
    graph.set(name, new Set(Object.keys(allDeps).filter((d) => workspaces.has(d))));
  }
...
for (const [packageName, { root, dir, packageJson }] of workspaces) {
  ...
  const allDeps = deps(packageJson, 'dependencies', 'devDependencies', 'optionalDependencies');
  const internalDeps = Object.keys(runtimeDeps).filter((d) => workspaces.has(d));
  ...
  if (root === 'cli') {
    ...
    if (!packageName.endsWith('-cli') && !NO_CLI_SUFFIX_ALLOWED.has(packageName)) {
      errors.push(`${dir}: CLI package name should end with "-cli" (or be explicitly allowed)`);
    }
  }
  ...
  if (root === 'packages') {
    if (hasBin) {
      errors.push(`${dir}: packages/ must not have a "bin" field — CLI entry points belong in cli/`);
    }
    if (!CLI_IO_MIGRATION_PENDING.has(packageName)) {
      for (const dep of CLI_IO_DEPS) {
        if (dep in runtimeDeps) {
          errors.push(`${dir}: packages/ must not depend on "${dep}" — CLI I/O belongs in cli/`);
        }
      }
    }
    for (const dep of SVELTE_DEPS) {
      if (dep in runtimeDeps) {
        errors.push(`${dir}: packages/ must not depend on "${dep}" in dependencies — use peerDependencies if optional`);
      }
    }
    ...
    for (const { file, module: mod } of hits) {
      errors.push(`${file}: packages/ must not import "${mod}" — terminal I/O belongs in cli/`);
    }
  }
  ...
  if (root === 'cli') {
    ...
    if (internalDeps.length === 0) {
      errors.push(`${dir}: cli/ must depend on at least one @univ-lehavre/atlas-* package — business logic belongs in packages/`);
    }
  }
  ...
  if (root === 'services') {
    ...
    if (internalDeps.length === 0) {
      errors.push(`${dir}: services/ must depend on at least one @univ-lehavre/atlas-* package — business logic belongs in packages/`);
    }
  }
  ...
  if (root === 'ui') {
    ...
    for (const dep of CLI_IO_DEPS) {
      if (dep in runtimeDeps) {
        errors.push(`${dir}: ui/ must not depend on "${dep}" — CLI I/O belongs in cli/`);
      }
    }
    // svelte must be a peerDependency, not a direct dependency
    if ('svelte' in runtimeDeps) {
      errors.push(`${dir}: ui/ must not depend on "svelte" in dependencies — use peerDependencies`);
    }
  }
  ...
  if (root !== 'sandbox') {
    for (const sandboxName of sandboxNames) {
      if (sandboxName in allDeps) {
        errors.push(`${dir}: must not depend on sandbox package "${sandboxName}"`);
      }
    }
  }
}
