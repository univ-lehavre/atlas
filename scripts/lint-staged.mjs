...
for (const [root, groupFiles] of groups) {
  ...
  try {
    // `--no-warn-ignored` : un fichier sciemment ignoré par la config
    // ESLint (ex. `config/**`) ne doit pas émettre le warning « File
    // ignored » qui ferait échouer le hook en `--max-warnings 0`.
    execFileSync("pnpm", ["exec", "eslint", "--no-warn-ignored", ...relFiles], {
      cwd: root,
      stdio: "inherit",
    });
  } catch {
