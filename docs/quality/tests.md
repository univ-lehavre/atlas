...
Les fichiers `*.test.ts` et `*.spec.ts` bénéficient de règles ESLint relâchées (mocks, assertions, `any` autorisé en local) — voir [Style de code → Fichiers de test](./code-style.md#fichiers-de-test).
