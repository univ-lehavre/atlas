...
> plutôt la [structure du monorepo](./monorepo.md) et les README de chaque app.

## Le réflexe : la carte des paquets

La question « pour comprendre tel paquet, lesquels dois-je lire ? » a une réponse
directe dans la [carte des paquets](./packages.md), générée depuis le code. Pour
...
- la [structure du monorepo](./monorepo.md) — les 8 catégories et leurs règles
  (vue d'ensemble) ;
- le [flux de données](./data-flow.md) — comment une donnée circule de bout en
...
- **[Effect](../glossary.md)** structure la logique métier : une fonction qui peut
  ...
  [ADR 0005](../decisions/0005-effect-pour-la-pf.md).
- **Les niveaux de documentation** : ce qui n'est pas dans la prose est dans la
  ...
  [la politique de documentation](../quality/documentation.md).
- **La séparation thin CLI / lib** : les outils en ligne de commande (`cli/`)
  ...
  [ADR 0008](../decisions/0008-clis-thins-logique-dans-packages.md).
