# 0007 — REDCap et Appwrite comme plateformes externes
...
Le risque clé est l'**enfermement métier** : si REDCap héberge des
données cliniques sensibles, le coût de migration en cas de changement
de stratégie devient prohibitif.

## Décision

- **REDCap** est utilisé pour les formulaires **administratifs uniquement**.
  Aucune donnée clinique n'est saisie dans REDCap depuis ce monorepo.
- **Appwrite** est utilisé comme BaaS (auth / base / stockage). Les
  apps déployées (`amarre`, `ecrin`, `find-an-expert`) utilisent le
  SDK officiel `node-appwrite` côté serveur (voir [ADR 0010](0010-node-appwrite-sdk-25.md)).

Toute extension du périmètre REDCap au-delà de l'administratif demande
une révision explicite de cet ADR.
...
**Bénéfices.** La séparation administratif / clinique est explicite et
...
- Les secrets REDCap et Appwrite sont inventoriés dans
  [docs/quality/security.md](../quality/security.md) avec leur procédure
  de rotation.
- Les SDK Appwrite sont pinnés et leur upgrade fait l'objet d'une PR
  isolée pour faciliter le rollback.
- Toute introduction de données cliniques dans REDCap nécessite une
