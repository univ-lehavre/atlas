# 0029 — Pipeline de collaborations : architecture V1 (plateforme DataOps, contrat Parquet)
...
- `packages/citation-fetch` + `packages/fetch-one-api-page` : ingestion de
  citations en [Effect](0005-effect-pour-la-pf.md), avec rate-limit (1 req/s) et
  pagination ;
...
- le mart est conçu **ré-dérivable by-design** (régénération / masquage), pour
  ...
  [ADR 0030](0030-rgpd-profilage-collaborations.md)).
...
(qui reste batch). Conformément à [ADR 0030](0030-rgpd-profilage-collaborations.md),
...
Conformément à [ADR 0008](0008-clis-thins-logique-dans-packages.md), la logique
...
consommateurs thins. Conformément à [ADR 0005](0005-effect-pour-la-pf.md), les
...
- **IA 100 % interne, sur CPU, sans aucun appel externe — _free tier_ inclus.**
  ...
  [ADR 0030](0030-rgpd-profilage-collaborations.md) — le caractère gratuit d'une
...
[ADR 0030](0030-rgpd-profilage-collaborations.md) (gate RGPD) avant tout
...
- **Nommage** : convention du dépôt — aucun nom de marque dans un identifiant.
  ...
  portée par un ADR dédié ; [ADR 0022](0022-naming-convention.md) ne traite que
...
- **Documentation** : la chaîne est documentée à plusieurs niveaux
  ([ADR 0025](0025-documentation-multi-niveaux.md)) — surface (PWA, README
...
- Le plan d'exécution phasé (une PR par phase, phase 0 = gate RGPD) vit dans
  `docs/plans/` et prend cet ADR + [ADR 0030](0030-rgpd-profilage-collaborations.md)
