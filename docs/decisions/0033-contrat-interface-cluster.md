...
Le pipeline de collaborations ([ADR 0029](0029-architecture-pipeline-collaborations.md))
...
L'outil étant **générique et multi-tenant** ([ADR 0031](0031-outil-generique-open-source.md)),
...
([ADR 0022](0022-naming-convention.md), [ADR 0031](0031-outil-generique-open-source.md)) ;
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| **Stockage objet S3**     | Un bucket dont le nom suit la convention `citation` (jamais la marque, [ADR 0022](0022-naming-convention.md)), accessible en **path-style**, avec un **Secret** d'identifiants S3 (`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / endpoint) généré par un **`ObjectBucketClaim`**. | Ceph RGW (déjà en prod) |
...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Namespaces**         | Les charges applicatives vivent dans `citation-ingest`, `citation-marts`, `citation-serving`, `citation-pwa` (convention [ADR 0022](0022-naming-convention.md) — jamais la marque).                 |
...
| **Métriques**          | Chaque service expose `/metrics` et déclare un `ServiceMonitor` ; aucune donnée personnelle dans les labels (cardinalité + RGPD, [ADR 0030](0030-rgpd-profilage-collaborations.md)).                |
...
Accepted (2026-06-02). Complète [ADR 0029](0029-architecture-pipeline-collaborations.md)
(architecture) et [ADR 0031](0031-outil-generique-open-source.md) (outil
...
- Le **nommage** suit [ADR 0022](0022-naming-convention.md) des deux côtés :
...
- Les **valeurs propres à une instance** (hôtes, IP, tailles, base légale)
  ...
  code générique ([ADR 0031](0031-outil-generique-open-source.md)).
