...
- 🔴 **Egress Internet** (bloquant) : le sync du snapshot (`s3://openalex`, AWS public) exige un accès Internet sortant ; le réseau est en default-deny ([0019](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0019-durcissement-reseau-cilium.md)). Une NetworkPolicy egress est posée sur le namespace **`dagster`** (où tourne le `aws s3 sync` du code-location), en `0.0.0.0/0` **restreint par ports 443/80** (pas de filtrage par plages IP S3 — piège connu sous Cilium).
...
- **Files (write) :** définition de l'asset + logique de sync S3 (option recommandée : `aws s3 sync s3://openalex/data/{works,authors} … --no-sign-request` dans un op conteneurisé — **téléchargement éphémère + push RGW**, car `s3://openalex` (AWS public) et `s3://citation` (RGW Ceph) sont **deux endpoints distincts**) ; **paramètre d'échelle** (limite de partitions/fichiers `.gz` pour le test local) ; Dockerfile de la code-location (AWS CLI inclus).
...
- **Validation :** `pnpm ci:checks` ; sur le banc Lima, sync **borné** (un seul `updated_date`) écrivant dans le RGW ; matérialisation vérifiée en mode dev (`dagster dev`). Note de **volumétrie** (≈ 330 Go gzippé / 1,6 To pour la prod) dans `docs/architecture/`.
- **Done criteria :** asset `raw_snapshot` défini, sync borné de works + authors vérifié, échelle pilotable par config.
