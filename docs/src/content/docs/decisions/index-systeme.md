...
| **CL-0113** | [Chaîne de livraison autonome : la branche `deploy`](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0113-chaine-livraison-branche-deploy.md)     | L'usine matérialise les digests sur `deploy` (projection `main ⊕ digests`) ; Argo CD suit `deploy` ; `main` garde ses placeholders. Paire de AT-0104.                          |
| **AT-0104** | [Mise en production par push Gitea](/atlas/decisions/0104-mise-en-production-push-gitea/)                                                                          | Le geste humain (push `main` → forge, par instance) + les **workflows de livraison** fournis par atlas (`.gitea/workflows/`). Miroir applicatif de CL-0113.                    |
