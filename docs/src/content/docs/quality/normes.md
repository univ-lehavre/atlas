...
| XI. **Logs**                  | Appliqué | Les **logs applicatifs** (du processus) vont sur stdout. `crf-logs` n'est **pas** un journal applicatif malgré son nom : c'est un **cache de données externes** (audit logs REDCap _fetchés_ par HTTP puis agrégés en analytics), découplé en _backing service_ injectable ([ADR 0040](/atlas/decisions/0040-caches-flux-backing-service-vs-fichier/), [#443](https://github.com/univ-lehavre/atlas/issues/443)). Le facteur XI ne s'y applique pas — le persister n'est pas un écart. |
...
télémétrie ([#309](https://github.com/univ-lehavre/atlas/issues/309)). Aucun n'est bloquant
