...
| V. **Build / release / run**  | Partiel     | Build distinct du run (SvelteKit/tsup). **Écart** : seul `sillage` a un Dockerfile ; les 5 autres apps et le service n'ont pas d'image de déploiement définie.                    |
...
| VII. **Port binding**         | Partiel     | Le service Hono lit `PORT` en env (`services/crf/src/server/env.ts`). **Écart** : pas de containerisation autonome pour 5 apps.                                                   |
...
| IX. **Disposability**         | Écart       | Les apps SvelteKit gèrent l'arrêt ; le **service Hono n'enregistre pas SIGTERM/SIGINT** (pas de fermeture propre du serveur).                                                     |
| X. **Dev/prod parity**        | Écart       | Sandboxes Docker reproduisent les backing services. **Écart** : `.nvmrc` non figé en version stricte, pas d'image prod pour le service.                                           |
...
| _ext._ **Sécurité / auth**    | Partiel     | Cookies durcis, CSP (`sveltekit-csp`), en-têtes, auth des apps. **Écart** : le **service CRF n'a pas de middleware d'auth** (routes ouvertes).                                    |
