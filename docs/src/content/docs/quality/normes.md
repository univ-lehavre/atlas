...
**Écart** : sur les deux pipelines, la boucle est **orchestrée mais non autonome
de bout en bout** — le CT (schedule + sensor) **déclenche** le réentraînement,
mais **aucun seuil ne le déclenche en boucle fermée** : l'armement et le réglage
du rythme restent à la main du déployeur. Côté `citation`, la dérive mesurée par
Evidently est journalisée (MLflow) et nourrit la _décision_ d'ajuster la cadence,
sans pour autant déclencher de réentraînement **automatiquement** ; côté
`mediawatch` (v1 sans Evidently ni MLflow), aucune mesure de dérive n'alimente
encore cette décision.
...
- **Continuous training _autonome_** — l'entraînement continu est orchestré
  (schedule + sensor) **sur les deux pipelines `dataops/` (`citation` et
  `mediawatch`)**, à parité (cf. section [MLOps](#mlops-et-modèle) et
  [ADR 0062](/atlas/decisions/0062-mlops-niveau-2-tracking-drift-ct/), section
  « généralisation aux pipelines DataOps »), mais la boucle fermée « seuil de
  dérive → réentraînement automatique » n'est pas encore en place : l'armement et
  la cadence restent à la main du déployeur.
