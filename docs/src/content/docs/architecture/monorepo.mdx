...
- **Hors du graphe pnpm** : `dataops/` n'est pas déclaré dans `pnpm-workspace.yaml`, donc pnpm, turbo et knip l'ignorent. `audit:structure` ne le contrôle pas non plus — sa discipline relève de ruff/pytest.
