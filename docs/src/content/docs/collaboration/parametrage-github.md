...
- **Vérifications de statut requises** — la PR ne peut fusionner que si la
  ...
  `dependency-review.yml`). Voir [Pipeline CI](../quality/ci-pipeline).
...
le [workflow de contribution](./workflow#8-merge).
...
local (voir [environnement local](./environnement-local#variables-denvironnement-et-secrets)).
...
voir [ADR 0017](../decisions/0017-releases-npm-oidc-deux-registres).
