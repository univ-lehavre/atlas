...
// Mock @sentry/sveltekit : le test ne vérifie que les en-têtes de sécurité, pas
// l'observabilité. Charger le vrai SDK (lourd, OpenTelemetry) via `await import`
// prend ~9 s sous charge CI → dépasse le testTimeout de 5 s (flaky). Le mock
// élimine la cause. `sentryHandle` doit rester un Handle valide pour `sequence`.
