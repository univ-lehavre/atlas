...
// complet. Le DSN est lu au runtime depuis `$env/dynamic/private`, donc le
// basculer ne demande aucun rebuild.
if (env.SENTRY_DSN) {
  Sentry.init({
    dsn: env.SENTRY_DSN,
 ...
 * pas déployé en prod. Aucune logique de session ici (lecture seule de
 * stats publiques) ; ce handle applique les mêmes security headers que le
