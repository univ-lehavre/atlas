...
// Exported for unit testing: the SvelteKit 2.61 `sequence()` helper used by
// `handle` requires the server-side request store (AsyncLocalStorage) which
// is only available inside the real runtime, so the Phase 9.2 security-header
// invariant is asserted directly against this inner handle.
