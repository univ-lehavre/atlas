 ...
 * @example
 * ```typescript
 * import { CrfHttpError, fromResponse } from '@univ-lehavre/atlas-crf-core/errors';
 ...
 */
export class CrfHttpError extends Data.TaggedError('CrfHttpError')<{
  /** HTTP status code */
 ...
 * @returns Promise resolving to CrfHttpError
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export const fromResponse = async (response: Response, url?: string): Promise<CrfHttpError> => {
  const body = await response.text().catch(() => undefined);
  return new CrfHttpError({
