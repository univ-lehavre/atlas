...
describe('Patterns', () => {
  ...
  describe('CRF_NAME_PATTERN', () => {
    it('should match valid REDCap names', () => {
      ...
      expect(CRF_NAME_PATTERN.test('survey1')).toBe(true);
    });

    it('should not match uppercase', () => {
      expect(CRF_NAME_PATTERN.test('Record_Id')).toBe(false);
    });

    it('should not match starting with number', () => {
      expect(CRF_NAME_PATTERN.test('1record')).toBe(false);
    });
  });
...
describe('Validators', () => {
  ...
  describe('isValidCrfName', () => {
    it('should validate REDCap names', () => {
      expect(isValidCrfName('record_id')).toBe(true);
      expect(isValidCrfName('Record_ID')).toBe(false);
    });
  });
