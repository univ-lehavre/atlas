...
describe('CrfToken', () => {
  describe('CRF_TOKEN_PATTERN', () => {
    it('should match 32 uppercase hex characters', () => {
      expect(CRF_TOKEN_PATTERN.test('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA')).toBe(true);
      expect(CRF_TOKEN_PATTERN.test('0123456789ABCDEF0123456789ABCDEF')).toBe(true);
      expect(CRF_TOKEN_PATTERN.test('DEADBEEF12345678DEADBEEF12345678')).toBe(true);
    });

    it('should not match lowercase hex characters', () => {
      expect(CRF_TOKEN_PATTERN.test('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')).toBe(false);
      expect(CRF_TOKEN_PATTERN.test('0123456789abcdef0123456789abcdef')).toBe(false);
    });
    ...
    it('should not match wrong length', () => {
      expect(CRF_TOKEN_PATTERN.test('AABBCCDD')).toBe(false);
      expect(CRF_TOKEN_PATTERN.test('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABB')).toBe(false);
      expect(CRF_TOKEN_PATTERN.test('')).toBe(false);
    });

    it('should not match non-hex characters', () => {
      expect(CRF_TOKEN_PATTERN.test('GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG1')).toBe(false);
      expect(CRF_TOKEN_PATTERN.test('ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ')).toBe(false);
    });
  });

  describe('CrfToken validator', () => {
    it('should accept valid tokens', () => {
      expect(() => CrfToken('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA')).not.toThrow();
      expect(() => CrfToken('0123456789ABCDEF0123456789ABCDEF')).not.toThrow();
    });

    it('should return the branded value', () => {
      const token = CrfToken('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA');
      expect(token).toBe('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA');
    });

    it('should throw for invalid tokens', () => {
      expect(() => CrfToken('invalid')).toThrow();
      expect(() => CrfToken('')).toThrow();
  ...
  describe('isValidToken', () => {
    ...
    it('should act as type guard', () => {
      ...
      if (isValidToken(value)) {
        // TypeScript should narrow to CrfToken
        const _token: CrfToken = value;
