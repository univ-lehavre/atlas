...
import { CitationSearchError } from "../errors.js";
...
export type { RateLimitInfo } from "@univ-lehavre/atlas-citation-fetch";
...
): Effect.Effect<ResolveAuthorsResult, CitationSearchError> => {
...
): Effect.Effect<readonly WorksResult[], CitationSearchError> =>
  authors.length === 0
    ? Effect.succeed([])
...
export const resolveAll = (
  row: ResearcherRow,
  config: CitationConfig,
): Effect.Effect<ResolveResult, CitationSearchError> =>
