...
import { CrfFetchError, CrfWriteError } from "../errors.js";
...
export interface CrfConnectionConfig {
  readonly url: string;
...
const makeClient = (
  config: CrfConnectionConfig,
): ReturnType<typeof createCrfClient> =>
  createCrfClient({
    url: CrfUrl(normalizeUrl(config.url)),
    token: CrfToken(config.token),
  });
...
export const fetchResearchers = (
  config: CrfConnectionConfig,
): Effect.Effect<readonly ResearcherRow[], CrfFetchError> => {
  ...
  return client
    ...
    .pipe(
      ...
      Effect.mapError((cause) => new CrfFetchError({ cause })),
    );
};
...
export const fetchResearcherData = (
  config: CrfConnectionConfig,
  userid: string,
): Effect.Effect<ResearcherData, CrfFetchError> => {
  const client = makeClient(config);
...
export const writeResearcherData = (
  ...
  userid: string,
  data: ResearcherData,
): Effect.Effect<void, CrfWriteError> => {
  const client = makeClient(config);
  return Effect.all(
    [
      client
        ...
        .pipe(
          Effect.asVoid,
          Effect.mapError((cause) => new CrfWriteError({ userid, cause })),
        ),
      client
...
export const writeFinalReferences = (
  ...
  return Effect.tryPromise<Uint8Array, CrfWriteError>({
    try: () =>
      generateCombinedPdf(
    ...
    catch: (cause) => new CrfWriteError({ userid, cause }),
  }).pipe(
...
export const downloadPublicationsFile = (
  config: CrfConnectionConfig,
  userid: string,
): Effect.Effect<ArrayBuffer, CrfFetchError> => {
  const client = makeClient(config);
  return client
    .exportFile("publications", userid)
    .pipe(Effect.mapError((cause) => new CrfFetchError({ cause })));
};
