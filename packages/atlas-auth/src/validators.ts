...
import { isEmail, isHexadecimal, normalizeEmail } from '@univ-lehavre/atlas-validators';
 ...
 * @param config - Domain validation configuration
 ...
 * ```
 */
export const validateSignupEmail = async (
  email: unknown,
  ...
  const domainRegex = new RegExp(config.allowedDomainsRegexp);
 ...
 * @example
 * ```typescript
 * const validUserId = validateUserId(locals.userId);
 * ```
 */
