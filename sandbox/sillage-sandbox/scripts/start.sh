...
#                                 the sillage dev server via webServer)
#
# Environment knobs :
...
==> Sandbox ready.
  ...
  The Playwright smoke spawns its own sillage dev server and kills it on
  exit, so http://localhost:5173 is NOT live right now. To use the app :

    pnpm -F @univ-lehavre/atlas-sillage dev
    → http://localhost:5173  (Ctrl+C to stop)

  Other URLs (still up — backed by docker) :
    - Mailpit (magic-links)    : http://localhost:8025
    - REDCap (projet sillage)   : http://localhost:8888
    - Appwrite console         : http://localhost:8091

  Stop everything : pnpm -F @univ-lehavre/atlas-sillage-sandbox stop
