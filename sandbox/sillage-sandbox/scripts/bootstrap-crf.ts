 ...
 *      token for `site_admin`. We treat project 1 as the sillage
 ...
 *      project 1 to "sillage" and flip it to development status (the
 *      metadata import below is rejected by REDCap on a production
 *      project).
 *   4. POST /api/?content=metadata&action=import to load the sillage
 *      data dictionary on top.
 *   5. Persist the token in `.env` as `CRF_API_TOKEN`.
 *
 * The data dictionary lives in `data-dictionaries/136-ecrin-v2-alpha.json`.
...
const importDataDictionary = async (
  ...
  // The export at data-dictionaries/136-ecrin-v2-alpha.json carries a few
...
const main = async (): Promise<void> => {
  ...
  if (before.project_title !== "sillage") {
    console.log(`==> Renaming project to 'sillage'`);
    await renameToSillage(crfUrl, token);
    const after = await exportProjectSettings(crfUrl, token);
    console.log(`  ✓ Now title='${after.project_title}'`);
  } else {
    console.log(`  • Project already named 'sillage'`);
  }

  console.log(`==> Importing sillage data dictionary`);
  await importDataDictionary(crfUrl, token);

  console.log(`==> Writing CRF_API_TOKEN to .env`);
  await upsertEnv({ CRF_API_TOKEN: token });
  console.log(`  ✓ .env updated\n`);
  console.log(`Done. REDCap 'sillage' project ready at ${crfUrl}`);
};
