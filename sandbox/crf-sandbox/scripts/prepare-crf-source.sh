...
# The REDCap ZIP itself is not committed; place it in cli/crf-openapi/upstream/
...
if [ -z "$ZIP_PATH" ]; then
  if [ -f "$SANDBOX_DIR/upstream/redcap${REDCAP_VERSION}.zip" ]; then
    ...
    ZIP_PATH="$REPO_ROOT/cli/crf-openapi/upstream/redcap${REDCAP_VERSION}.zip"
  fi
fi
...
if [ ! -f "$ZIP_PATH" ]; then
  ...
  echo "Put redcap${REDCAP_VERSION}.zip in cli/crf-openapi/upstream/ or set REDCAP_ZIP."
  exit 1
fi
