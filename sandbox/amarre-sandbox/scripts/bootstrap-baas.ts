#!/usr/bin/env -S tsx
/**
 * Full headless bootstrap of the self-hosted Appwrite stack used by amarre.
 *
 ...
 *      scopes amarre needs (users.read/write, sessions.write, account).
...
// scope serveur. Pour le flow magic-link d'amarre (createMagicURLToken +
...
const main = async (): Promise<void> => {
  ...
  const root: RootAccountConfig = {
    email: env["APPWRITE_ROOT_EMAIL"] || "admin@amarre.local",
    password: env["APPWRITE_ROOT_PASSWORD"] || "",
    name: env["APPWRITE_ROOT_NAME"] || "Amarre Sandbox Admin",
  };
  ...
  const teamId = env["APPWRITE_ORG_ID"] || "org-amarre-sandbox";
  const teamName = env["APPWRITE_ORG_NAME"] || "Amarre Sandbox";
  console.log(`==> Ensuring organisation`);
  await ensureTeam(ctx, teamId, teamName);

  const projectCfg: ProjectConfig = {
    projectId: env["APPWRITE_PROJECT_ID"] || "amarre",
    name: env["APPWRITE_PROJECT_NAME"] || "amarre",
    teamId,
