...
Empêche de déployer du code applicatif sur une pré-image ``scholar-network-deps-base``
...
deps-base, et le build de code (``FROM scholar-network-deps-base:<vieux-hash>``) casse —
...
code-location manifest). Trois entrées, TOUTES dans ``dataops/scholar-network-dagster/`` :

  1. ``uv.lock`` (fichier entier ; chaque wheel porte son sha256). C'est l'entrée
     DOMINANTE — un bump de dépendance la change.
  ...
  3. ``model_provenance.py`` (fichier entier) — la provenance du modèle ONNX :
     ``HF_REPO``/``HF_REVISION`` + les sha256 des fichiers. Un changement de
...
Le tag de la base ENCODE ce digest : ``registry:80/scholar-network-deps-base:<SHA_DEPS>``
...
``ARG DEPS_REF=registry:80/scholar-network-deps-base:dev`` (défaut banc). En régime réel,
...
──────────────────────────────── MODE DUAL ───────────────────────────────────
  ...
  • ONLINE (``--check-registry``, poste de contrôle) : EN PLUS, interroge le
    registre pour vérifier que ``scholar-network-deps-base:<SHA_DEPS>`` existe. Si
    absente → ERREUR BLOQUANTE (les deps ont changé, la base n'a pas été rebuildée/poussée).

Le tag construit est LOGIQUE (``registry:80/scholar-network-deps-base:<SHA_DEPS>``), mais
...
# scripts/ → le parent est dataops/scholar-network-dagster/. On y trouve uv.lock, le
# Dockerfile et src/scholar_network_dagster/model_provenance.py. Chemins DÉRIVÉS (pas de
...
# `scholar-network` ; d'autres code-locations pourront réutiliser ce même calcul
# plus tard (leur deps-base a les mêmes entrées SAUF le modèle ONNX, absent chez
# certaines — d'où model_provenance CONDITIONNEL à sa présence). ────────────────
...
def compute_sha_deps(
    ...
    *,
    ...
    code-locations sans modèle ONNX n'ont pas cette entrée — on ne l'inclut PAS
    dans le hash (plutôt qu'inclure une chaîne vide, qui serait indistinguable
    d'un fichier vide). Défaut ``scholar-network`` → toujours présent.
    ...
    if mediatype is None:
        return [
            Finding(
                ...
                f"      -f dataops/scholar-network-dagster/Dockerfile -t {tag} --push dataops/\n"
...
def _load_inputs(workspace: str, cl: str) -> tuple[str, str, str | None]:
    ...
    # model_provenance CONDITIONNEL à sa présence (neutralité : code-locations sans
    # modèle ONNX). Défaut `scholar-network` → présent.
...
def _report(findings: list[Finding]) -> int:
    ...
    if errors:
        print(
            ...
            "`scholar-network-deps-base:<SHA_DEPS>` avant de déployer le code.",
            file=sys.stderr,
        )
        return 1
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Garde-fou de fraîcheur de la pré-image scholar-network-deps-base "
        "(ADR cluster 0110).",
    )
    parser.add_argument(
        "--check-registry",
    ...
    parser.add_argument(
        "--code-location",
        default=_DEFAULT_CL,
        help="Code-location (défaut scholar-network). D'autres réutiliseront ce calcul.",
    )
    parser.add_argument(
        "--workspace",
        default=_WORKSPACE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            ...
            "(workspace dataops/scholar-network-dagster/).",
            file=sys.stderr,
        )
        return 2
