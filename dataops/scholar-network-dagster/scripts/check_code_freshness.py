"""Garde-fou de fraîcheur de l'IMAGE DE CODE ``scholar-network-dagster`` (build hors cluster).
...
code-location (``src/``, tranche ``code`` du Dockerfile) et OUBLIE de rebuilder/pousser
l'image → le cluster déploie une image PÉRIMÉE (le drift « en retard de N commits » vécu).
Ce check DÉTECTE l'écart et alerte de façon bloquante.
...
cible ``code`` du Dockerfile — CE QUI CHANGE quand « le code a changé » :
  ...
  2. ``pyproject.toml`` : les métadonnées/points d'entrée du paquet (``uv pip install .``).
  3. La TRANCHE ``code`` du ``Dockerfile`` (de ``FROM ${DEPS_REF} … AS code`` à la fin) :
     si les instructions de build du code changent (COPY, USER…).
...
scholar-network N'A PAS de projet dbt (chaîne linéaire brut→passes→profil, produit =
vecteur pgvector, ADR 0103) — aucune entrée dbt n'est donc hachée, à la différence de
citation.
...
Le tag de l'image de code peut ENCODER ce digest :
``registry:80/scholar-network-dagster:<SHA_CODE>`` (immuable, jamais ``:latest`` —
ADR cluster 0006). ``--print-tag`` l'imprime — source de vérité unique consommée par
``build-code.sh`` (``--tag``). NB : le tag COURANT du pipeline est ``:dev`` (déployé par
digest, pas par tag) ; ``SHA_CODE`` sert la fraîcheur et un éventuel tag immuable, pas à
renommer le flux existant.
...
──────────────────────────────── MODE DUAL ───────────────────────────────────
  ...
  • ONLINE (``--check-registry``, poste) : EN PLUS, interroge le registre pour vérifier
    que ``scholar-network-dagster:<SHA_CODE>`` existe. Absente → ERREUR BLOQUANTE (le
    code a changé sans rebuild+push de l'image).
...
_DEFAULT_CL = "scholar-network"
_REGISTRY_LOGICAL = "registry:80"
_REGISTRY_ENDPOINT_DEFAULT = "registry:80"

# scripts/ → parent = dataops/scholar-network-dagster/ (le workspace de la code-location).
...
def extract_code_stage(dockerfile_text: str) -> str:
    ...
    capte les instructions de build du CODE (COPY src, uv pip install, USER) sans
    numéros de ligne fragiles.
...
def _iter_source_files(root: str) -> list[str]:
    ...
    Ignore les artefacts non-source (``__pycache__``, ``.pyc``, ``target/``, ``logs/``)
...
def _load_code_entries(workspace: str, cl: str) -> list[tuple[str, str]]:
    ...
    Ordre déterministe : pyproject → tranche Dockerfile `code` → src/ (trié).
    Lève ``FileNotFoundError`` / ``ValueError`` (propagées en exit 2 par main).
    scholar-network N'EMBARQUE PAS de projet dbt (chaîne linéaire, ADR 0103) : aucune
    entrée dbt à hacher, à la différence de citation.
    """
...
def _report(findings: list[Finding]) -> int:
    ...
    if errors:
        print(
            ...
            "`scholar-network-dagster:<SHA_CODE>` (deploy/build-code.sh) avant de déployer.",
            file=sys.stderr,
        )
        return 1
    print(
    ...
    for f in findings:
        if f.level == ERROR:
            out.append(
                Finding(
                    ERROR,
                    f"image de code ABSENTE au registre : {tag}. Le code a changé (src / "
                    "Dockerfile:code) sans rebuild+push. Builder puis pousser l'image AVANT de "
                    "déployer :\n"
                    "    dataops/scholar-network-dagster/deploy/build-code.sh "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Garde-fou de fraîcheur de l'image de code scholar-network-dagster (ADR 0110).",
    )
    parser.add_argument(
        "--check-registry",
        action="store_true",
    ...
    parser.add_argument(
        "--code-location",
        default=_DEFAULT_CL,
        help="Code-location (défaut scholar-network).",
    )
    parser.add_argument(
        "--workspace",
        default=_WORKSPACE,
