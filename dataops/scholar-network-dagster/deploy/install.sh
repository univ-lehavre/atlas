#!/usr/bin/env bash
# Installation de la code-location Dagster « scholar-network » sur le BANC — preuve
...
# (`__SCHOLAR_NETWORK_IMAGE_DIGEST__`, `__SCHOLAR_NETWORK_IMAGE__`) que le cluster
# remplit par le digest immuable de l'image de CODE qu'il build in-pod sur la pré-image
# (ADR cluster 0110). Build de l'image, injection du digest et réconciliation prod sont
# des gestes `cluster` (frontière ADR 0033/0094). Ici, on prouve le MÊME code applicatif
# sur le banc ; seul le backing S3 diffère (ADR 0036/0085).
#
# Usage :
...
image="$registry/scholar-network-dagster"
...
#   1a. cible `deps`  → la pré-image `scholar-network-deps-base` (LOURDE, egress : deps +
...
deps_ref="$registry/scholar-network-deps-base:$tag"
