#!/usr/bin/env bash
# Build + push de l'IMAGE DE CODE « scholar-network-dagster » (cible `code` du Dockerfile).
#
...
# l'overlay prod (`__SCHOLAR_NETWORK_IMAGE_DIGEST__`) puis Argo CD déploie par digest immuable.
#
...
#     --tag         tag de l'image de code (défaut `dev` → registry:80/scholar-network-dagster:dev)
...
if [ -z "$deps_ref" ]; then
  ...
  deps_ref="${endpoint}/scholar-network-deps-base:${sha_deps}"
fi

# ── 2. Tag de PUSH de l'image de code (endpoint substitué) ────────────────────
push_tag="${endpoint}/scholar-network-dagster:${tag}"
meta_file="$(mktemp -t scholar-network-code-meta.XXXXXX.json)"
trap 'rm -f "$meta_file"' EXIT

echo "── build-code : image de CODE scholar-network-dagster ──"
...
echo "  digest : $digest"
echo "  → à injecter dans overlays/prod (__SCHOLAR_NETWORK_IMAGE_DIGEST__) pour le déploiement GitOps."
