...
from pageviews_dagster import lakehouse, last_run, lineage
...
def latest_run_parts(entries: list[dict], mart_subdir: str = _FORECAST_SUBDIR) -> dict[str, int]:
    """Sélectionne, par mois (``dt=``), les parts du DERNIER ``run=`` par RÉCENCE (ADR 0101).
    ...
    run de chaque mois** — « dernier run gagne ». « Dernier » = run au ``ModTime`` S3 le
    plus récent (PAS l'ordre lexical du ``run=``, un uuid4 aléatoire — ADR 0101), via
    :func:`last_run.latest_run_by_day`. Les runs obsolètes restent sur disque (nettoyage
    par lifecycle S3, contrat infra).
    ...
    ``{clé_absolue: octets}`` des seules parts retenues.
    """
