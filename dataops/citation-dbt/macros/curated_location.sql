{#
  Chemin de sortie IMMUABLE d'un modèle curated (étape 3.2).

  Forme : <curated_root>/<model>/dt=<curated_dt>/run=<curated_run>/part.parquet
  Défaut : s3://citation/curated/<model>/dt=…/run=…/part.parquet
  ...
  `curated_root` est une var (défaut `s3://citation/curated`) : le smoke hermétique
...
{{ return(
    var("curated_root", "s3://citation/curated") ~ "/" ~ model_name
    ~ "/dt=" ~ var("curated_dt")
