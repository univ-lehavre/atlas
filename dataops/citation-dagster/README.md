...
maintient en service. Côté DEV, le geste est **un simple push de code** : **l'image se
build et se déploie automatiquement**, sans `docker buildx` manuel.

> **Mise en place — la preuve banc de bout en bout (run from-scratch) est en cours.** La
> chaîne décrite ici est le **design** du déploiement événementiel ; son exécution
> complète n'est pas encore prouvée par un run banc.

**Pousser le code suffit (déploiement événementiel).** Un **`git push` sur `atlas/atlas`**
qui touche `dataops/citation-dagster/` déclenche le **webhook Gitea #2** (celui du
**build** de code — distinct du webhook #1 qui, sur un push `cluster/apps`, déclenche le
**déploiement** Argo CD). Ce push est capté par **Argo Events** : un `Sensor` **dérive** la
code-location du chemin modifié, **build l'image dans le cluster** (BuildKit rootless),
**lit le digest** produit et fait le **write-back** de `apps/citation.yaml` **par
`@sha256`** dans le repo Gitea `cluster/apps` ; le webhook #1 fait alors réconcilier Argo
CD. Le **déploiement se fait par digest, jamais par tag** : le SHA court reste le tag
*lisible* (traçabilité commit→image), le `@sha256` est l'**ancre d'immuabilité**. Le
mécanisme complet vit dans l'**ADR cluster
[0095](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0095-build-applicatif-evenementiel-in-cluster.md)
§1.b** — non dupliqué ici.

**Build local = preuve/dev seulement.** Le `docker buildx` manuel **n'est pas le canal de
déploiement** : il ne sert qu'à un **build de preuve applicative locale / dev** (le profil
`bench`, ADR cluster
[0085](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0085-preuves-applicatives-local-path.md)).
L'image se construit en **arm64** pour le banc Lima. Le **contexte de build est `dataops/`**
(et non `dataops/citation-dagster/`) : l'image embarque la code-location **et** le projet
dbt frère `citation-dbt/`, requis par les assets dbt. D'où le `-f` explicite (et le
`dataops/.dockerignore` qui borne le contexte) :
...
**Frontière atlas ↔ cluster.** L'orchestrateur (côté cluster) **découvre** cette image via
son workspace et la tient en service ; c'est elle qui exécute nos assets lors des runs. Les
manifestes de déploiement (Deployment + Service gRPC, `ObjectBucketClaim` pour l'accès S3,
patch du workspace Dagster, `Application` Argo CD) vivent dans [`deploy/`](deploy/) et sont
...
`atlas` **déclare et fournit** ; le **cluster** **valide, build et remplit** le digest dans
les placeholders d'image de l'overlay prod (frontière ADR cluster
[0094](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0094-frontiere-deploiement-applicatif.md)).
La procédure pas à pas — flux événementiel, preuve locale et armement du CT — est le
**runbook** [`deploy/RUNBOOK.md`](deploy/RUNBOOK.md) ; la **preuve applicative** se joue sur
un banc **mono-nœud local-path** (ADR cluster
[0085](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0085-preuves-applicatives-local-path.md)),
le détail des cibles et profils étant du ressort de `cluster`.
