...
**produit livrable** est une **image conteneur** (un serveur gRPC), qu'Argo CD déploie et
maintient en service, **par digest immuable** (`@sha256`, jamais un tag mutable).

**Deux images au cycle de vie disjoint (pré-image de build, ADR cluster
[0110](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0110-preimage-de-build-et-build-in-pod.md)).**
Le [`Dockerfile`](Dockerfile) a **deux cibles** :

- **cible `deps`** → la pré-image `citation-deps-base` : tout ce qui touche Internet
  (dépendances système, wheels du lock, extensions DuckDB, modèle ONNX). Buildée
  **rarement** (à chaque bump de `uv.lock`), **hors cluster** sur le poste de contrôle
  (là où le réseau est disponible), et poussée au registre par
  [`deploy/build-deps-base.sh`](deploy/build-deps-base.sh). Le tag encode un digest des
  dépendances (`SHA_DEPS`) ; le garde-fou
  [`scripts/check_deps_base_freshness.py`](scripts/check_deps_base_freshness.py) refuse de
  déployer du code sur une pré-image périmée.
- **cible `code`** (défaut) → l'image applicative : `FROM ${DEPS_REF}` + le code frais +
  `dbt parse`. **Aucun accès réseau** (`UV_OFFLINE=1`) → elle se build **dans le cluster**
  (in-pod, air-gappé) sans jamais rien télécharger. C'est tout l'intérêt du découpage.

**Le contexte de build est `dataops/`** (et non `dataops/citation-dagster/`) : l'image
embarque la code-location **et** le projet dbt frère `citation-dbt/`, requis par les assets
dbt. D'où le `-f` explicite (et le `dataops/.dockerignore` qui borne le contexte). Sur le
banc local (poste avec réseau), [`deploy/install.sh`](deploy/install.sh) build les deux
cibles à la suite (preuve applicative locale, profil `bench`, ADR cluster
[0085](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0085-preuves-applicatives-local-path.md)) :

```bash
# 1. la pré-image (cible deps, egress) — sur le poste :
docker buildx build --target deps \
  -f dataops/citation-dagster/Dockerfile \
  -t registry:80/citation-deps-base:<SHA_DEPS> --push dataops/
# 2. l'image de code (cible code, zéro egress) :
docker buildx build --target code \
  --build-arg DEPS_REF=registry:80/citation-deps-base:<SHA_DEPS> \
  -f dataops/citation-dagster/Dockerfile \
...
**Frontière atlas ↔ cluster.** `atlas` **déclare et fournit** (le Dockerfile à deux cibles,
la pré-image, les placeholders d'image de l'overlay prod) ; le **cluster** **valide, build
l'image de code in-pod, lit le digest et le remplit** dans les placeholders, puis Argo CD
réconcilie depuis **Gitea** — jamais de `kubectl apply` manuel (frontière ADR cluster
[0094](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0094-frontiere-deploiement-applicatif.md),
correction d'état par le code [ADR cluster
0046](https://github.com/univ-lehavre/cluster/blob/main/docs/decisions/0046-corriger-le-code-pas-l-etat.md)).
Le **« comment » du déploiement** (moteur de build in-pod, déclenchement) est du ressort de
`cluster` (ADR 0110 et son plan d'implémentation), **pas documenté ici** — `atlas` ne
fabrique ni ne résout l'image de production.
