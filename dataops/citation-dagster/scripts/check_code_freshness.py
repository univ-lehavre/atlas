"""Garde-fou de fraîcheur de l'IMAGE DE CODE ``citation-dagster`` (build hors cluster).
...
rebuilder/pousser l'image → le cluster déploie une image PÉRIMÉE (le drift « citation en
...
cible ``code`` du Dockerfile — CE QUI CHANGE quand « le code a changé » :
  ...
  2. ``citation-dbt/`` (récursif) : le projet dbt copié dans l'image (``models``,
     ``macros``, ``seeds``, ``dbt_project.yml``…). ATTENTION : dossier FRÈRE de la
     code-location (contexte de build = ``dataops/``), pas sous ``citation-dagster/``.
...
Le tag de l'image de code peut ENCODER ce digest : ``registry:80/citation-dagster:<SHA_CODE>``
...
──────────────────────────────── MODE DUAL ───────────────────────────────────
  ...
  • ONLINE (``--check-registry``, poste) : EN PLUS, interroge le registre pour vérifier
    que ``citation-dagster:<SHA_CODE>`` existe. Absente → ERREUR BLOQUANTE (le code a
...
# scripts/ → parent = dataops/citation-dagster/ (le workspace de la code-location).
...
def _load_code_entries(workspace: str, cl: str) -> list[tuple[str, str]]:
    ...
    ``citation-dbt`` est un dossier FRÈRE (contexte de build ``dataops/``) : on remonte
    d'un cran depuis le workspace de la code-location.
    """
    module = cl.replace("-", "_") + "_dagster"
    dataops_ctx = os.path.dirname(workspace)  # dataops/ (parent de citation-dagster/)
...
def _report(findings: list[Finding]) -> int:
    ...
    if errors:
        print(
            ...
            "`citation-dagster:<SHA_CODE>` (deploy/build-code.sh) avant de déployer.",
            file=sys.stderr,
        )
        return 1
    print(
        ...
        if f.level == ERROR:
            out.append(
                Finding(
                    ...
                    "déployer :\n"
                    "    dataops/citation-dagster/deploy/build-code.sh "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Garde-fou de fraîcheur de l'image de code citation-dagster (ADR 0110).",
    )
    parser.add_argument(
        "--check-registry",
        action="store_true",
    ...
    parser.add_argument(
        "--code-location",
        default=_DEFAULT_CL,
        help="Code-location (défaut citation).",
    )
    parser.add_argument(
        "--workspace",
        default=_WORKSPACE,
