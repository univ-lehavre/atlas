...
Empêche de déployer du code applicatif sur une pré-image ``citation-deps-base``
...
deps-base, et le build de code (``FROM citation-deps-base:<vieux-hash>``) casse —
...
code-location manifest). Trois entrées, TOUTES dans ``dataops/citation-dagster/`` :
...
Le tag de la base ENCODE ce digest : ``registry:80/citation-deps-base:<SHA_DEPS>``
...
``ARG DEPS_REF=registry:80/citation-deps-base:dev`` (défaut banc). En régime réel,
...
──────────────────────────────── MODE DUAL ───────────────────────────────────
  ...
  • ONLINE (``--check-registry``, poste de contrôle) : EN PLUS, interroge le
    registre pour vérifier que ``citation-deps-base:<SHA_DEPS>`` existe. Si absente
    → ERREUR BLOQUANTE (les deps ont changé, la base n'a pas été rebuildée/poussée).

Le tag construit est LOGIQUE (``registry:80/citation-deps-base:<SHA_DEPS>``), mais
...
# scripts/ → le parent est dataops/citation-dagster/. On y trouve uv.lock, le
# Dockerfile et src/citation_dagster/model_provenance.py. Chemins DÉRIVÉS (pas de
...
# `citation` ; `mediawatch`/`pageviews` pourront réutiliser ce même calcul plus
...
def compute_sha_deps(
    ...
    *,
    ...
    indistinguable d'un fichier vide). Défaut ``citation`` → toujours présent.
    ...
    if mediatype is None:
        return [
            Finding(
                ...
                f"      -f dataops/citation-dagster/Dockerfile -t {tag} --push dataops/\n"
...
def _load_inputs(workspace: str, cl: str) -> tuple[str, str, str | None]:
    ...
    # n'ont pas de modèle ONNX). Défaut `citation` → présent.
...
def _report(findings: list[Finding]) -> int:
    ...
    if errors:
        print(
            ...
            "`citation-deps-base:<SHA_DEPS>` avant de déployer le code.",
            file=sys.stderr,
        )
        return 1
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Garde-fou de fraîcheur de la pré-image citation-deps-base (ADR cluster 0110).",
    )
    parser.add_argument(
        "--check-registry",
        action="store_true",
    ...
    parser.add_argument(
        "--code-location",
        default=_DEFAULT_CL,
        help="Code-location (défaut citation). mediawatch/pageviews réutiliseront ce calcul.",
    )
    parser.add_argument(
        "--workspace",
        default=_WORKSPACE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            ...
            "(workspace dataops/citation-dagster/).",
            file=sys.stderr,
        )
        return 2
