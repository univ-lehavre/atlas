#!/usr/bin/env bash
# Build + push de l'IMAGE DE CODE « citation-dagster » (cible `code` du Dockerfile).
#
...
# l'overlay prod (`__CITATION_IMAGE_DIGEST__`) puis Argo CD déploie par digest immuable.
#
...
#     --tag         tag de l'image de code (défaut `dev` → registry:80/citation-dagster:dev)
...
cl_dir="$here/.."                                  # dataops/citation-dagster/
...
if [ -z "$deps_ref" ]; then
  ...
  deps_ref="${endpoint}/citation-deps-base:${sha_deps}"
fi

# ── 2. Tag de PUSH de l'image de code (endpoint substitué) ────────────────────
push_tag="${endpoint}/citation-dagster:${tag}"
meta_file="$(mktemp -t citation-code-meta.XXXXXX.json)"
trap 'rm -f "$meta_file"' EXIT

echo "── build-code : image de CODE citation-dagster ──"
...
echo "  digest : $digest"
echo "  → à injecter dans overlays/prod (__CITATION_IMAGE_DIGEST__) pour le déploiement GitOps."
