#!/usr/bin/env bash
# Build + push de la PRÉ-IMAGE de build « citation-deps-base » (ADR cluster 0110).
#
...
# La base est taguée `registry:80/citation-deps-base:<SHA_DEPS>`, où SHA_DEPS est le
...
# `--print-tag` imprime `registry:80/citation-deps-base:<SHA_DEPS>` (nom logique).
...
push_tag="${endpoint}/citation-deps-base:${sha_deps}"
