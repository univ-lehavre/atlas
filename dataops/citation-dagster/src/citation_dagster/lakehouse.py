...
def connect(cfg: DuckDBS3Config | None = None) -> duckdb.DuckDBPyConnection:
    ...
    # PARALLÉLISE. Le gros consommateur est l'asset mart_eunicoast, qui lit le lac PAR LOTS
    # (~5M works PROJETÉS/lot = quelques Go — jamais les colonnes lourdes, ADR 0094) ; l'aval
    # dbt (~70k works) est petit. `memory_limit=24GB` borne la RAM ; au-delà, DuckDB déborde ses
    # opérateurs dans temp_directory (emptyDir monté, cf. definitions._SPILL_MOUNT) au lieu d'OOM
    # — filet, quasi gratuit. `threads=32` parallélise le filtrage d'un lot sans réserver tous
    # les cœurs du nœud au repos (dimensionnement révisé depuis l'ancienne approche « tout
    # OpenAlex en RAM » qui exigeait 60 threads / 64GB). Le pod de run RÉSERVE ces ressources
    # (requests/limits, cf. definitions._RUN_RESOURCES) → le scheduler place et cadre. Tout
    # dérivé de l'env (ADR 0023 : le banc léger baisse encore).
