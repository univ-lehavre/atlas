...
NB : ``last_run.py`` est le **jumeau** de ceux de ``mediawatch-dagster`` et
``pageviews-dagster`` (``dataops`` n'a pas de package Python partagé, ADR 0055). citation
n'en exerce que la sélection ``N-1`` du drift : il n'inclut donc que :func:`previous_complete_run`
(et ses aides), pas le ``latest_run_by_day`` que les deux autres emploient pour leur manifest."""
