...
def test_build_dbt_vars_injects_event_day_and_default_ref_source(monkeypatch) -> None:
    monkeypatch.delenv("MEDIAWATCH_REF_SOURCE", raising=False)
    vars_ = dbt.build_dbt_vars("run-abc", event_day="2026-01-01")
    # event_day borne le scan du brut ET sert de période immuable (curated_dt).
...
def test_build_dbt_vars_relays_ingested_ref_source(monkeypatch) -> None:
    ...
    assert dbt.build_dbt_vars("r", event_day="2026-01-01")["ref_source"] == "ingested"
