...
class _FakeCon:
    """`_load_run_summary` lit n_articles_pred (df) et served_mode (rows) du run retrouvé
    dans le glob `run=<id>/`. (La sélection N-1 ne passe plus par DuckDB mais par rclone —
    voir `_fake_rclone` — donc plus de branche `DISTINCT run` ici.)"""
...
def _make_fake_rclone(runs_modtime):
    """Renvoie un `_run_rclone` factice : le lsjson du mart forecast (runs + ModTime) pour la
    sélection N-1 (`_previous_run`), vide pour toute autre racine (ex. `raw/gkg` de
...
def test_baseline_is_previous_run_by_modtime(monkeypatch):
    ...
    # l'ancien `ORDER BY run` + `previous[-1]` aurait choisi "mmm" (avant "zzz" en lexical).
    by_run = {
    ...
    res = d.check_forecast_drift("mediawatch", "aaa")
