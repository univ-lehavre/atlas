...
#   3. invariants de contrat : OPENLINEAGE_URL présent ; resources.limits et
#      PodDisruptionBudget présents (QoS/anti-SPOF, issue #400) ; en prod, image
#      jamais en `:dev`, envFrom du ConfigMap BUCKET_* (mediawatch-datalake),
#      MEDIAWATCH_S3_SECRET/_CONFIGMAP posés et alignés sur l'OBC (pods de run), et
...
for o in "${overlays[@]}"; do
  ...
  if [ "$o" = "prod" ]; then
    ...
    # Pods de RUN : definitions.py branche la source S3 via MEDIAWATCH_S3_SECRET /
    # MEDIAWATCH_S3_CONFIGMAP (le nom vit dans le Python, hors du rendu kustomize —
    ...
    # présence ET que MEDIAWATCH_S3_SECRET pointe le MÊME nom que l'ObjectBucketClaim.
    obc_name="$(printf '%s' "$rendered" | awk '
      $1=="kind:" && $2=="ObjectBucketClaim"{f=1}
      f && $1=="name:"{print $2; exit}')"
    for v in MEDIAWATCH_S3_SECRET MEDIAWATCH_S3_CONFIGMAP; do
    ...
    if [ -n "$obc_name" ]; then
      # La valeur de MEDIAWATCH_S3_SECRET (ligne `value:` suivant son `name:`) doit
      # être le nom de la claim — sinon le pod de run vise un Secret inexistant.
      # Le nom est un item de liste YAML : `- name: MEDIAWATCH_S3_SECRET` → $2/$3 ;
      # la valeur suit sur sa propre ligne : `value: mediawatch-datalake` → $1/$2.
      s3_secret_val="$(printf '%s' "$rendered" | awk '
        $2=="name:" && $3=="MEDIAWATCH_S3_SECRET"{f=1; next}
        f && $1=="value:"{print $2; exit}')"
      if [ "$s3_secret_val" != "$obc_name" ]; then
        echo "✗ prod : MEDIAWATCH_S3_SECRET ($s3_secret_val) ≠ ObjectBucketClaim ($obc_name)." >&2
        fail=1
      else
        echo "✓ MEDIAWATCH_S3_SECRET aligné sur l'OBC ($obc_name)"
      fi
    fi
    ...
    # bruyante ici.
    for ph in \
      "__MEDIAWATCH_IMAGE_DIGEST__:kustomization.yaml" \
      "__MEDIAWATCH_IMAGE__:patch-s3-envfrom.yaml"; do
