#!/usr/bin/env bash
# Build + push de l'IMAGE DE CODE « mediawatch-dagster » (cible `code` du Dockerfile).
#
...
# l'overlay prod (`__MEDIAWATCH_IMAGE_DIGEST__`) puis Argo CD déploie par digest immuable.
#
...
#     --tag         tag de l'image de code (défaut `dev` → registry:80/mediawatch-dagster:dev)
...
cl_dir="$here/.."                                  # dataops/mediawatch-dagster/
...
if [ -z "$deps_ref" ]; then
  ...
  deps_ref="${endpoint}/mediawatch-deps-base:${sha_deps}"
fi

# ── 2. Tag de PUSH de l'image de code (endpoint substitué) ────────────────────
push_tag="${endpoint}/mediawatch-dagster:${tag}"
meta_file="$(mktemp -t mediawatch-code-meta.XXXXXX.json)"
trap 'rm -f "$meta_file"' EXIT

echo "── build-code : image de CODE mediawatch-dagster ──"
...
echo "  digest : $digest"
echo "  → à injecter dans overlays/prod (__MEDIAWATCH_IMAGE_DIGEST__) pour le déploiement GitOps."
