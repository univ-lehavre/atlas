...
Empêche de déployer du code applicatif sur une pré-image ``mediawatch-deps-base``
...
deps-base, et le build de code (``FROM mediawatch-deps-base:<vieux-hash>``) casse —
...
code-location manifest). Trois entrées, TOUTES dans ``dataops/mediawatch-dagster/`` :
...
Le tag de la base ENCODE ce digest : ``registry:80/mediawatch-deps-base:<SHA_DEPS>``
...
``ARG DEPS_REF=registry:80/mediawatch-deps-base:dev`` (défaut banc). En régime réel,
...
──────────────────────────────── MODE DUAL ───────────────────────────────────
  ...
  • ONLINE (``--check-registry``, poste de contrôle) : EN PLUS, interroge le
    registre pour vérifier que ``mediawatch-deps-base:<SHA_DEPS>`` existe. Si absente
    → ERREUR BLOQUANTE (les deps ont changé, la base n'a pas été rebuildée/poussée).

Le tag construit est LOGIQUE (``registry:80/mediawatch-deps-base:<SHA_DEPS>``), mais
...
# scripts/ → le parent est dataops/mediawatch-dagster/. On y trouve uv.lock, le
# Dockerfile et src/mediawatch_dagster/model_provenance.py. Chemins DÉRIVÉS (pas de
...
# `mediawatch` ; `mediawatch`/`pageviews` pourront réutiliser ce même calcul plus
...
def compute_sha_deps(
    ...
    *,
    ...
    indistinguable d'un fichier vide). Défaut ``mediawatch`` → toujours présent.
    ...
    if mediatype is None:
        return [
            Finding(
                ...
                f"      -f dataops/mediawatch-dagster/Dockerfile -t {tag} --push dataops/\n"
...
def _load_inputs(workspace: str, cl: str) -> tuple[str, str, str | None]:
    ...
    # n'ont pas de modèle ONNX). Défaut `mediawatch` → présent.
...
def _report(findings: list[Finding]) -> int:
    ...
    if errors:
        print(
            ...
            "`mediawatch-deps-base:<SHA_DEPS>` avant de déployer le code.",
            file=sys.stderr,
        )
        return 1
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--code-location",
        default=_DEFAULT_CL,
        help="Code-location (défaut mediawatch). mediawatch/pageviews réutiliseront ce calcul.",
    )
    parser.add_argument(
        "--workspace",
        default=_WORKSPACE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            ...
            "(workspace dataops/mediawatch-dagster/).",
            file=sys.stderr,
        )
        return 2
