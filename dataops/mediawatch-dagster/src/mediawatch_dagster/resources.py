"""Configuration des accès stockage objet pour la code-location « mediawatch ».

À la différence de « citation » (source S3→S3 via deux remotes rclone), la source
de mediawatch est un **serveur HTTP** (les fichiers 15 minutes de GDELT) : le
téléchargement se fait en HTTP (``httpx``), pas par rclone. Un **seul remote**
rclone est donc nécessaire — ``ceph``, le lakehouse interne souverain (RGW Ceph
en prod, SeaweedFS au banc) — pour l'écriture du brut et du ``manifest``.

Les identifiants proviennent du Secret de l'``ObjectBucketClaim`` (variables
``AWS_ACCESS_KEY_ID`` / ``AWS_SECRET_ACCESS_KEY``) et l'endpoint du ConfigMap
associé (``BUCKET_HOST`` / ``BUCKET_PORT`` / ``BUCKET_NAME``) — mêmes noms que
« citation », pour que le contrat banc ↔ prod soit identique.

Le ``rclone.conf`` est rendu **à l'exécution** depuis l'environnement et écrit
dans un répertoire temporaire : aucun identifiant n'est committé ni présent dans
l'image.
"""
...
def render_rclone_config(target: CephTarget) -> str:
    ...
    ici (la source GDELT est HTTP, téléchargée par ``httpx`` en amont).
    """
    return "\n".join(
        [
            "[ceph]",
