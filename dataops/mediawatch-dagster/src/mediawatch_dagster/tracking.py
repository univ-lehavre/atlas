"""Instrumentation MLflow de mediawatch (modèle de prévision, ADR 0081).

Première instrumentation MLOps de mediawatch : à chaque matérialisation du modèle de
prévision, on logge un *run* MLflow (params + métriques de la validation honnête). C'est
du **code applicatif** (frontière ADR 0033) : le SERVEUR MLflow est fourni par le socle ;
ici on lit seulement ``MLFLOW_TRACKING_URI`` et on instrumente — comme le lineage lit
``OPENLINEAGE_URL``.

Réduit par rapport à ``citation`` (pas de model registry : aucun modèle vendoré ; le
gradient boosting est ré-entraîné à chaque run, sa provenance est la graine figée +
``uv.lock``). **No-op sans ``MLFLOW_TRACKING_URI``** (code-location chargeable et asset
matérialisable hors cluster — tests/CI hermétiques, ADR 0057). Aucune PII (ADR 0030) ;
identifiants ``mediawatch_*``, jamais une marque (ADR 0022).
...
# Expérience DÉDIÉE au modèle de prévision — convention `mediawatch`, jamais une marque.
EXPERIMENT_FORECAST = "mediawatch_university_forecast"
...
def mlflow_config_from_env(env: dict | None = None) -> MlflowConfig | None:
    ...
    défaut ``mediawatch_university_forecast``."""
